...
laceBtn.onclick = async () => {
    if (window.cardano?.lace) {
        try {
            laceApi = await window.cardano.lace.enable();
        ...
        if (used && used.length > 0) {
            ...
            try {
                ...
                if (CSL && CSL.Address && /^[0-9a-fA-F]+$/.test(addr)) {
                    ...
                    const bytes = Uint8Array.from(addr.match(/.{1,2}/g).map((x) => parseInt(x, 16)));
...
async function signAndSubmitTxWithWallet(walletApi, cborHex, walletLabel, resultDiv) {
    try {
        ...
        if (origVkeys)
            for (let i = 0; i < origVkeys.len(); i++)
                mergedVkeys.add(origVkeys.get(i));
        const walletVkeys = walletWitnessSet.vkeys();
        if (walletVkeys)
            for (let i = 0; i < walletVkeys.len(); i++)
                mergedVkeys.add(walletVkeys.get(i));
        if (mergedVkeys.len() > 0)
            mergedWitnessSet.set_vkeys(mergedVkeys);
        // Build the signed transaction
        const signedTx = CSL.Transaction.new(unsignedTx.body(), mergedWitnessSet, unsignedTx.auxiliary_data());
